    ...
    <div class="container max-w-ev-container mx-auto px-4 md:px-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 lg:gap-12">
            @foreach($blogs as $blog)
                <a href="{{ $blog['url'] }}" target="_blank" class="group flex flex-col h-full bg-white transition-all duration-300">
                    {{-- Image Container --}}
                    <div class="aspect-16/10 sm:aspect-video overflow-hidden rounded-xl md:rounded-2xl mb-4 md:mb-6 shadow-sm group-hover:shadow-md transition-all duration-300">
